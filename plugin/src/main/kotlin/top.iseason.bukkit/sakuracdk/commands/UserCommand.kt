...
fun userCommand() {
    command(
        ...
        executor { params, sender ->
            ...
            dbTransaction {
                val cdkResult = CDKs.selectAll().where { CDKs.id eq cdk }.limit(1).firstOrNull()
                ...
                if (!cdkYml.allowRepeat()) {
                    if (!Records.select(Records.group).where { Records.group eq group and (Records.uid eq uniqueId) }
                            .limit(1).empty()) {
                        throw ParmaException(Lang.command__user_has_accepted)
                    }
                    if (cdkYml is NormalCDKYml) {
                        val exist = Records.select(Records.id.count()).where { Records.group eq group }
